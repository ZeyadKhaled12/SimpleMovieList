...
class MoviesBloc extends Bloc<MoviesEvent, MoviesState> {
  ...
  FutureOr<void> _getNowPlayingMovies(
    ...
    final result = await getNowPlayingMoviesUC.call(const NoParameters());
    result.fold(
  ...
  FutureOr<void> _getPopularMovies(GetPopularMoviesEvent event, Emitter<MoviesState> emit) async{
    final result = await getNowPopularMoviesUC.call(const NoParameters());
    result.fold(
            (l) => emit(state.copyWith(
  ...
  FutureOr<void> _getTopRatedMovies(GetTopRatedMoviesEvent event, Emitter<MoviesState> emit) async{
    final result = await getTopRatedMoviesUC.call(const NoParameters());
    result.fold(
            (l) => emit(state.copyWith(
